import { O as getCsfFactoryAnnotations, S as composeStory, bt as UnsupportedViewportDimensionError, i as isStory, n as getStoryChildren } from "./csf-DhnoPZiI.js";
import { i as STORYBOOK_TEST_PROVIDE_KEY } from "./chunk-OHAJYSSA-wKIPJ0Wc.js";
import { inject } from "vitest";
//#region node_modules/storybook/dist/_browser-chunks/chunk-ASKQZAOS.js
...
//#endregion
//#region node_modules/@storybook/addon-vitest/dist/vitest-plugin/test-utils.js
var DEFAULT_VIEWPORT_DIMENSIONS = {
	...
	if (percentagePattern.test(value)) {
		...
		throw new UnsupportedViewportDimensionError({
			dimension,
			value
		});
	}
...
var convertToFilePath = (url) => url.replace(/^file:\/\//, "").replace(/^\/+([a-zA-Z]:)/, "$1").replace(/%20/g, " "), testStory = ({ exportName, story, meta, skipTags, storyId, componentPath, testName, componentName }) => async (context) => {
	...
	try {
		runConfig = inject("storybook/test-provided") ?? { a11y: !0 };
	} catch {}
	let ghostStoriesEnabled = !1;
	try {
		ghostStoriesEnabled = inject("storybook/core-ghost-stories") ?? !1;
	} catch {}
	let renderAnalysisEnabled = !1;
	try {
		renderAnalysisEnabled = inject("storybook/core-render-analysis") ?? !1;
	} catch {}
	let shouldRunA11yTests = !!runConfig.a11y, initialGlobals = {
		[STORYBOOK_TEST_PROVIDE_KEY]: runConfig,
		...ghostStoriesEnabled ? { ghostStories: { enabled: !0 } } : {},
		...renderAnalysisEnabled ? { renderAnalysis: { enabled: !0 } } : {},
		a11y: { manual: !shouldRunA11yTests }
	}, composedStory = composeStory(storyAnnotations, annotations.meta, { initialGlobals }, annotations.preview ?? globalThis.globalProjectAnnotations, exportName);
